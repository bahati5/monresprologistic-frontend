        ...
        return (
          ...
          >
            ...
            <div className="min-w-0 space-y-2 p-2 md:hidden">
              {rates.length === 0 ? (
                <p className="p-2 text-xs text-muted-foreground">Aucun tarif actif sur cette ligne.</p>
              ) : (
                rates.map((rate) => {
                  const rid = String(rate.id ?? "");
                  const sel = shipLineRateId === rid;
                  const sm = (rate.shipping_mode as { name?: unknown } | undefined)?.name;
                  const dt = effectiveDelayLabelForRate(rate) || "";
                  const price = wizardRateUnitPrice(rate);
                  return (
                    <div
                      key={rid || `l${String(line.id)}-m${String(rate.shipping_mode_id ?? "")}`}
                      className={cn("space-y-2 rounded-md border border-border p-3", sel && "bg-primary/5")}
                    >
                      ...
                      <p className="text-sm font-medium">{displayLocalized(sm)}</p>
                      <p className="text-xs text-muted-foreground">{dt.trim() ? dt : "—"}</p>
                      <p className="text-sm font-semibold tabular-nums">{formatMoney(price)}</p>
                      <Button
                        type="button"
                        size="sm"
                        variant={sel ? "default" : "outline"}
